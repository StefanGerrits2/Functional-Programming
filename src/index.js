...
function cleanData(results) {
    for(let key in results) { // Loop over every value
        categoryName = results[key].categoryName.value.charAt(0).toUpperCase() + results[key].categoryName.value.slice(1); // Change first character to uppercase + full string except first character
